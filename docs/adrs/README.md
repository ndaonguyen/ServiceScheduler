...
| [0004](0004-inter-service-communication-strategy.md) | Inter-service communication strategy after module extraction | Accepted | 2026-07-08 | Post-extraction cross-service reads default to sync HTTP/gRPC + resilience; upgrade specific paths to local read models built from events only when measured availability requires it. Async request-reply for queries is never used. |
