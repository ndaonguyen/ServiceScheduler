...
| [0005](0005-postgresql-over-document-database.md) | PostgreSQL over document databases for the primary datastore | Accepted | 2026-07-08 | PostgreSQL + EF Core (Npgsql) chosen as the primary store. Load-bearing decision: `EXCLUDE USING gist` on `appointments` (with `btree_gist`) enforces the no-double-booking guarantee at the schema layer. Document databases (MongoDB, CosmosDB) rejected — no equivalent atomic exclusion, no schemaless requirement, no document-shaped aggregates, and community-maintained Identity providers. |
| [0006](0006-project-per-module-physical-structure.md) | Project-per-module physical structure, module schemas, provider-owned contracts | Accepted | 2026-07-09 | Refines [0001](0001-modular-monolith.md): module boundaries become compiler-enforced via one class-library per module under `src/` + a NetArchTest suite; cross-module query ports move to the provider's `Contracts` project; one shared `AppDbContext` with a Postgres schema per module. |
| [0007](0007-shared-resource-scheduling-ownership.md) | Resource-owned scheduling when a shared resource gains a second consumer | Proposed | 2026-07-11 | Dormant, trigger-conditioned. When a shared resource (technician/bay) gains a second reservation-writing consumer, its schedule ownership moves to the resource service (Workforce/Fleet) via a generic `Reservation` aggregate; the `EXCLUDE USING gist` no-overlap constraint moves with it; `Appointment` references a reservation id; cross-service booking becomes an orchestrated saga with release-reservation compensations and TTL holds. Conditionally supersedes [0003](0003-appointment-as-scheduling-source-of-truth.md) for shared resources on acceptance. |
