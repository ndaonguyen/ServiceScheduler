...
decisions behind this shape see the [ADRs](../../docs/adrs/).

```mermaid
flowchart TB
    Client["API Client<br/>(Web · Mobile · OpenAPI)"]

    subgraph Process ["Single .NET process (OTel-instrumented)"]
        ...
        subgraph BB ["BuildingBlocks (shared)"]
            direction LR
            Cross["ICurrentUser"]
    ...
    classDef contract stroke:#5aa085,stroke-width:1.5px,stroke-dasharray:4 2
    classDef highlight stroke:#3466c2,stroke-width:2.5px
...
- **Dashed arrows** are cross-module references that go **only through
  ...
  [ADR-0001](../../docs/adrs/0001-modular-monolith.md) enforces and
...
| **Cross-module writes** | Not present today. When added, will use events per [ADR-0002](../../docs/adrs/0002-events-for-inter-module-communication.md). |
