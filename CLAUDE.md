...
(`.template.config/template.json`, `sourceName: ServiceScheduler`). Vertical-slice CQRS over a
**lightweight in-process mediator** (`ServiceScheduler.Application/Messaging`, no MediatR), an optional
**React + Vite SPA** (`source/ServiceScheduler.Api/ClientApp`), a Widgets servicescheduler, and `/health`. Ships
with GitHub Actions CI and Claude skills.

> **Persistence:** EF Core + **PostgreSQL** (`Npgsql`) for runtime queries only. `AppDbContext`
> lives in `ServiceScheduler.Infrastructure/Persistence`; the Widgets slice persists through it via
...
> Migration code lives in `source/ServiceScheduler.Infrastructure/Migrations/`; the design-time context is
...
> source/ServiceScheduler.Infrastructure --startup-project source/ServiceScheduler.Api`; commit the generated files.
...
> (`ServiceScheduler.Api/Security`); `AuthCookies` flags + `JwtBearerEvents.OnMessageReceived` (in `Program.cs`,
...
> (`ServiceScheduler.Infrastructure/Security`) issues/rotates/revokes. Config = `Jwt` section (`SigningKey`
